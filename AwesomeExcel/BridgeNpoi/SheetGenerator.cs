...
internal class SheetGenerator
{
    ...
    private void AutoSizeColumns(_NPOI.ISheet npoiSheet, _Excel.Sheet excelSheet)
    {
        ...
        string getDateTimeFormat(_Excel.Sheet excelSheet, int columnIndex)
        {
        ...
        int getStringCellLength(int rowIndex, int columnIndex)
        {
            ...
            _Excel.Cell cell = row.Cells[columnIndex];
            string str = (string)cell.Value;
            return str?.Length ?? 0;
        }

        int getLongestStringCharactersCount(_Excel.Sheet excelSheet, int columnIndex)
        {
            int longestString = 0;
        ...
        int getColumnMinimumWidth(_Excel.Sheet excelSheet, int columnIndex)
        {
            int columnMinimumWidth = 0;

            _Excel.Column _column = excelSheet.Columns[columnIndex];

            if (_column.ColumnType == _Excel.ColumnType.DateTime)
            {
            ...
            else if (_column.ColumnType == _Excel.ColumnType.String)
            {
